...
# 可选 GitHub 代理前缀（如 https://your.proxy/gh/）。
# 设了就优先走代理，失败自动回退 GitHub 直连；不设则直连。
GH_PROXY="${TAKO_GH_PROXY:-}"

# curl 下载到 stdout：代理优先，失败回退直连。$1=github url, 其余=curl 额外参数
gh_curl() {
	_url=$1
	shift
	if [ -n "$GH_PROXY" ]; then
		if curl -fsSL "$@" "${GH_PROXY%/}/${_url}"; then
			return 0
		fi
		echo "  proxy failed, falling back to direct GitHub" >&2
	fi
	curl -fsSL "$@" "$_url"
}
...
fetch_difft() {
	...
	if test "$ext" = "zip"; then
		gh_curl "$url" -o "$tmp/difft-$plat.zip" || return 1
		unzip -q "$tmp/difft-$plat.zip" -d "$tmp/difft-$plat"
	else
		gh_curl "$url" | tar -xzf - -C "$tmp/difft-$plat" || return 1
	fi
...
fetch_rg() {
	...
	if test "$ext" = "zip"; then
		gh_curl "$url" -o "$tmp/rg-$plat.zip" || return 1
		unzip -q "$tmp/rg-$plat.zip" -d "$tmp/rg-$plat"
	else
		gh_curl "$url" | tar -xzf - -C "$tmp/rg-$plat" || return 1
	fi
