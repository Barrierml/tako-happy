...
function loadRipgrepNative() {
    ...
    // Packaged binary fallback (preferred over system rg: deterministic,
    // and avoids logging noise on stdout from the system-rg branch).
    if (fs.existsSync(binaryPath)) {
        return createRipgrepWrapper(binaryPath);
    }

    // System ripgrep fallback (last resort before mock).
