...
`)
    ...
    if (showHelp) {
      console.log(`
${chalk.bold('tako-remote')} - Claude Code On the Go

${chalk.bold('Usage:')}
  tako-remote [options]         Start Claude with mobile control
  tako-remote auth              Manage authentication
  tako-remote resume            Resume a previous session by session ID
  tako-remote codex             Start Codex mode
  tako-remote gemini            Start Gemini mode (ACP)
  tako-remote acp               Start a generic ACP-compatible agent
  tako-remote connect           Connect AI vendor API keys
  tako-remote sandbox           Configure and manage OS-level sandboxing
  tako-remote notify            Send push notification
  tako-remote daemon            Manage background service that allows
                                  to spawn new sessions away from your computer
  tako-remote doctor            System diagnostics & troubleshooting

${chalk.bold('Examples:')}
  tako-remote                    Start session
  tako-remote resume cmmij8      Resume a previous session by session ID
  tako-remote --yolo             Start with bypassing permissions
                                  sugar for --dangerously-skip-permissions
  tako-remote --chrome           Enable Chrome browser access for this session
  tako-remote --no-chrome        Disable Chrome even if default is on
  tako-remote --no-sandbox       Disable sandbox for this session
  tako-remote --js-runtime bun   Use bun instead of node to spawn Claude Code
  tako-remote --claude-env ANTHROPIC_BASE_URL=http://127.0.0.1:3456
                                 Use a custom API endpoint (e.g., claude-code-router)
  tako-remote acp gemini         Start Gemini via generic ACP runner
  tako-remote acp -- opencode --acp
                                 Start a custom ACP command
  tako-remote acp opencode --verbose
                                 Print raw ACP backend/envelope events
  tako-remote auth login --force Authenticate
  tako-remote doctor             Run diagnostics

${chalk.bold('Tako supports ALL Claude options!')}
  Use any claude flag with tako-remote as you would with claude. Our favorite:
